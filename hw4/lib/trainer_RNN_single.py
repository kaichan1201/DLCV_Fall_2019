...
class Trainer:
    def __init__(self, model, train_loader, val_loader, optim, criterion, args, writer, use_cuda):
        ...
        self.rnn_batch_num = self.args.rnn_batch_num

        self.iters = 0
        self.max_iter = len(self.train_loader) * self.epochs // self.rnn_batch_num + 1
        self.best_acc = 0
        self.total_cnt = 0
        self.correct_cnt = 0
    ...
    def train(self):
        for epoch in range(1, self.epochs + 1):
            ...
            for idx, (video, lbl) in enumerate(self.train_loader):
                ...
                lbls.append(lbl.view(-1))

                if (idx+1) % self.rnn_batch_num == 0 or (idx+1) == len(self.train_loader):
                    self.iters += 1
                    current_iter += 1
                    ...
                    if current_iter % 25 == 0 or current_iter == len(self.train_loader) // self.rnn_batch_num:
                        print('Epoch [{}][{}/{}], Loss: {:.4f}'.format(epoch, current_iter,
                                                                       len(self.train_loader) // self.rnn_batch_num,
                                                                       loss.item()))
                    '''reset list'''
                    feats = []
                    lbls = []
    ...
    def evaluate(self, epoch):
        ...
        with torch.no_grad():  # do not need to calculate information for gradient during eval
            for idx, (video, gt) in enumerate(self.val_loader):
                ...
                feat = self.E(video)
                feats.append(feat)
                gts.append(gt.view(-1))

                if (idx + 1) % self.rnn_batch_num == 0 or (idx + 1) == len(self.val_loader):
                    gts = torch.cat(gts)
